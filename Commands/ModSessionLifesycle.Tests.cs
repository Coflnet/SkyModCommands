...
public class ModSessionLifesycleTests
{
    ...
    public async Task PremiumTierFilterUpgrades(bool replace)
    {
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            socket.Flips.Clear();
            // simulate multiple users with same settings
            await lifesycle.FlipSettings.Update(settings);
            ...
            socket.Flips.Count.Should().Be(2);
            lifesycle.SessionInfo.SessionTier = AccountTier.PREMIUM_PLUS;
            await lifesycle.FlipProcessor.ProcessFlip(CreateSampleFlip());
            socket.Flips.Count.Should().Be(2);
    ...
    private static LowPricedAuction CreateSampleFlip()
    {
        return new LowPricedAuction
        {
            Auction = new()
            {
                ...
                StartingBid = 5300000,
                UId = Random.Shared.Next(1000000, 9999999),
                Bin = true,
            },
